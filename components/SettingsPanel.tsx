    ...
    <>
      ...
      >
        ...
        <div className="space-y-6">
          ...
          {/* Offline Status */}
          <div className="pt-2">
            <div className={`flex items-center gap-2 text-sm ${isOfflineReady ? 'text-green-600 dark:text-green-400' : 'text-[rgb(var(--text-tertiary))]'}`}>
              <div className={`w-2 h-2 rounded-full ${isOfflineReady ? 'bg-green-500' : 'bg-gray-400 animate-pulse'}`}></div>
              {isOfflineReady ? 'التطبيق جاهز للعمل بدون إنترنت' : 'جاري تجهيز وضع العمل بدون إنترنت...'}
            </div>
          </div>
        </div>
