...
const RelatedSectionsSkeleton: React.FC = () => (
    <div className="space-y-3 animate-pulse">
        <div className="h-4 bg-[rgb(var(--background-tertiary))] rounded w-1/3"></div>
        <div className="space-y-2">
            <div className="h-3 bg-[rgb(var(--background-tertiary))] rounded w-3/4"></div>
            <div className="h-3 bg-[rgb(var(--background-tertiary))] rounded w-1/2"></div>
            <div className="h-3 bg-[rgb(var(--background-tertiary))] rounded w-2/3"></div>
        </div>
    </div>
);
...
const RelatedSections: React.FC<RelatedSectionsProps> = ({ currentSection, allSections }) => {
  ...
  return (
    <div ref={ref} className="mt-8 pt-6 border-t border-[rgb(var(--border-secondary))] related-sections-container min-h-[100px]">
      {isLoading ? (
        ...
        <>
          <h3 className="text-base font-semibold text-[rgb(var(--text-primary))] mb-3">
            أقسام ذات صلة
          </h3>
            ...
            {related.map((relatedTitle, index) => {
              ...
              <li key={index}>
                <a
                  ...
                  className="flex items-center text-sm text-[rgb(var(--primary))] hover:text-[rgb(var(--primary-hover))] hover:underline transition-colors duration-200 group"
                >
